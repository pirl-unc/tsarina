...
This document describes how the perseo CTA gene set is built, filtered, and maintained.
...
The perseo CTA set is built as an **unbiased union** from multiple published CT antigen databases, then **systematically filtered** using Human Protein Atlas tissue expression data to retain only genes with reproductive-restricted expression.
