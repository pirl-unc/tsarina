...
def test_multiallelic_exact_restrictions_use_best_prediction(monkeypatch):
    hits = pd.DataFrame(
        {
            "peptide": ["MAGEAPEP1"],
            "mhc_restriction": ["HLA-A*02:01;HLA-B*07:02"],
            ...
            "is_monoallelic": [False],
            "pmid": ["224"],
            "cell_line_name": ["multi-allelic sample"],
        }
    )
    monkeypatch.setattr("tsarina.ms_evidence.load_public_ms_hits", lambda peptides, **kw: hits)
...
def test_proteoform_registry_groups_loaded_beyond_the_anchors():
    # _CTA_GROUPS is derived from the oncoref-synced registry mirror, not the
...
def test_proteoform_groups_mirror_in_sync_with_oncoref():
    # The mirror is generated by scripts/sync_proteoform_groups.py; if oncoref
    # is importable, assert it hasn't drifted from the source of truth.
    oncoref = pytest.importorskip("oncoref")
    ...
    source = {
        label: tuple(sorted(members)) for label, members in oncoref.proteoform_symbol_map().items()
    }
