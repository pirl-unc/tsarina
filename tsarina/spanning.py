...
#: The four well-known identical-protein groups, kept explicit as the offline
#: fallback for :func:`_load_proteoform_groups` and as the carriers of the curated
#: non-symbol aliases below (NY-ESO-1, GAGED2, XAGE1C-E, …).
...
#: Curated non-symbol aliases for the anchor groups (NY-ESO-1, GAGED2, the
#: XAGE1C-E Ensembl synonyms, …) — preserved verbatim and overlaid last so anchor
#: resolution is byte-identical to the hand-maintained map.
...
def _load_proteoform_groups() -> dict[str, tuple[str, ...]]:
    """Identical-protein CGA groups (``{label: member symbols}``).

    Sourced from the ``cancerdata``-derived registry mirror
    (``data/proteoform-groups.csv`` — 15 families incl. the 12-member CT47A),
    regenerated by ``scripts/sync_proteoform_groups.py``. The four anchor groups
    are the offline fallback if the file is missing/unreadable, so grouping never
    silently disappears.
    """
    groups = dict(_ANCHOR_GROUPS)
    path = Path(__file__).parent / "data" / "proteoform-groups.csv"
    try:
        df = pd.read_csv(path)
        for label, sub in df.groupby("proteoform_id"):
            groups[str(label)] = tuple(str(s) for s in sub["member_symbol"])
...
_CTA_GROUPS: dict[str, tuple[str, ...]] = _load_proteoform_groups()

#: Compact (uppercase-alphanumeric) alias -> grouped CTA label.  Single source of
#: truth for resolving NY-ESO-1 / XAGE1 / SSX4 / CT47A / … names to their
#: identical-protein group.
_GROUP_ALIAS_TO_LABEL: dict[str, str] = _build_group_alias_map(_CTA_GROUPS)
...
def _resolve_ctas(
    ...
    from .loader import canonical_default_mask
    ...
    df = df[canonical_default_mask(df)]
