...
Storage location: ``~/.ctabase/`` (override with ``CTABASE_DATA_DIR`` env var).

Typical usage::

    from ctabase.downloads import register, get_path, fetch, list_datasets
...
CLI::
    ...
    ctabase-data register iedb /data/mhc_ligand_full.csv
    ctabase-data register cedar /data/cedar-mhc-ligand-full.csv
    ctabase-data fetch hpv16
    ctabase-data list
    ctabase-data path iedb
"""
...
_DEFAULT_DATA_DIR = Path.home() / ".ctabase"


def data_dir() -> Path:
    """Return the ctabase data directory, creating it if needed."""
    d = Path(os.environ.get("CTABASE_DATA_DIR", str(_DEFAULT_DATA_DIR)))
    ...
    if name not in FETCHABLE_DATASETS:
        if name in MANUAL_DATASETS:
            ...
            raise ValueError(
                ...
                f"  {info['download_url']}\n"
                f"Then register with: ctabase-data register {name} /path/to/{info['expected_filename']}"
            )
...
def get_path(name: str) -> Path:
    ...
    if entry is None:
        ...
        if name in FETCHABLE_DATASETS:
            hint = f"\n  Fetch with: ctabase-data fetch {name}"
        elif name in MANUAL_DATASETS:
            info = MANUAL_DATASETS[name]
            hint = (
                f"\n  Download from: {info['download_url']}"
                f"\n  Then register: ctabase-data register {name} /path/to/file"
            )
